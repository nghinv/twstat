...
//IDEIncludeCommand.TQA_XSS_DIR = "file:///java/exo-working/user-extensions--scripts/twstat-master/twstat-master/test/testcases/";
...
IDEIncludeCommand.prototype.prepareUrl = function(includeUrl) {
    ...
    if (window.location.href.indexOf("selenium-server") >= 0) {
        ...
        //preparedUrl = absolutify(includeUrl, IDEIncludeCommand.TQA_XSS_DIR);
      preparedUrl = absolutify(includeUrl, "file://" + editor.suiteTreeView.currentTestCase.file.path);
...
IDEIncludeCommand.prototype.injectIncludeTestCommandsx = function(locator,includeCmdRow,  testRows) { 
    // Rewrite logic for Selenium IDE by Jerry Qian
    ...
    for (var i = 1 ; i < testRows.length; i++) {
	  ...
	  new_target=getUnescapedText(new_target);
	  new_value=getUnescapedText(new_value);
	  
	  LOG.debug("Including new command >>new_cmd=" + new_cmd + " >>new_target=" + new_target + " >>new_value=" + new_value);
...
function getUnescapedText(inputText)
{
  if (inputText.toString().search("\&") < 0)
  {
    return inputText
  }
...
Selenium.prototype.doBegin$Templatex$ = function(locator){
    LOG.info("Begin Template " + locator);	
};

var beginTemplateIndex = null;
Selenium.prototype.doBegin$Template$ = function(locator){
    LOG.info("Begin Template " + locator);
	beginTemplateIndex = testCase.debugContext.debugIndex;
};


Selenium.prototype.doEnd$Templatex$ = function(locator){
...
Selenium.prototype.doEnd$Template$ = function(locator){
    ...
    testCase.commands = 
	testCase.commands.slice(0, beginTemplateIndex).concat(
    ...
    testCase.debugContext.debugIndex = beginTemplateIndex-1;
...
xmlTestData.prototype.load = function(xmlloc) {
	...
	var xmlHttpReq = IDEIncludeCommand.prototype.getIncludeDocumentBySynchronRequest(xmlloc);
